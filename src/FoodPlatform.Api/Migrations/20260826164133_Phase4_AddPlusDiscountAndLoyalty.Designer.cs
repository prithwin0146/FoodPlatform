// <auto-generated />
using System;
using FoodPlatform.Api.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace FoodPlatform.Api.Migrations
{
    [DbContext(typeof(FoodPlatformDbContext))]
    [Migration("20260826164133_Phase4_AddPlusDiscountAndLoyalty")]
    partial class Phase4_AddPlusDiscountAndLoyalty
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.30")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.Auction", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<decimal>("BidIncrement")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.Property<decimal?>("BuyNowPrice")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.Property<string>("CameraId")
                        .HasColumnType("text");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("NOW()");

                    b.Property<decimal?>("CurrentBid")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.Property<string>("Description")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<DateTime?>("EndsAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("ImageUrl")
                        .HasColumnType("text");

                    b.Property<int?>("MenuItemId")
                        .HasColumnType("integer");

                    b.Property<int>("RestaurantId")
                        .HasColumnType("integer");

                    b.Property<int>("SoftCloseSeconds")
                        .HasColumnType("integer");

                    b.Property<decimal>("StartingPrice")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.Property<DateTime?>("StartsAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<int?>("WinningBidId")
                        .HasColumnType("integer");

                    b.Property<int?>("WinningUserId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("MenuItemId");

                    b.HasIndex("RestaurantId")
                        .HasDatabaseName("IX_Auctions_RestaurantId");

                    b.HasIndex("Status")
                        .HasDatabaseName("IX_Auctions_Status");

                    b.HasIndex("WinningUserId");

                    b.ToTable("Auctions");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.Bid", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<decimal>("Amount")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.Property<int>("AuctionId")
                        .HasColumnType("integer");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("NOW()");

                    b.Property<int>("UserId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("AuctionId")
                        .HasDatabaseName("IX_Bids_AuctionId");

                    b.HasIndex("UserId");

                    b.ToTable("Bids");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.FavouriteRestaurant", b =>
                {
                    b.Property<int>("UserId")
                        .HasColumnType("integer");

                    b.Property<int>("RestaurantId")
                        .HasColumnType("integer");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("NOW()");

                    b.HasKey("UserId", "RestaurantId");

                    b.HasIndex("RestaurantId");

                    b.HasIndex("UserId")
                        .HasDatabaseName("IX_FavouriteRestaurants_UserId");

                    b.ToTable("FavouriteRestaurants");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.GiftCard", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("NOW()");

                    b.Property<decimal>("InitialAmount")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<int?>("PurchasedByUserId")
                        .HasColumnType("integer");

                    b.Property<decimal>("RemainingBalance")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.Property<string>("StripePaymentIntentId")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.HasKey("Id");

                    b.HasIndex("Code")
                        .IsUnique()
                        .HasDatabaseName("IX_GiftCards_Code");

                    b.HasIndex("PurchasedByUserId");

                    b.ToTable("GiftCards");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.GiftCardUsage", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<decimal>("AmountUsed")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.Property<int>("GiftCardId")
                        .HasColumnType("integer");

                    b.Property<int>("OrderId")
                        .HasColumnType("integer");

                    b.Property<DateTime>("UsedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("NOW()");

                    b.HasKey("Id");

                    b.HasIndex("GiftCardId");

                    b.HasIndex("OrderId");

                    b.ToTable("GiftCardUsages");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.LoyaltyAccount", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<decimal>("AccountCreditBalance")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("NOW()");

                    b.Property<int>("StampCount")
                        .HasColumnType("integer");

                    b.Property<int>("StampsRequiredForReward")
                        .HasColumnType("integer");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("UserId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("UserId")
                        .IsUnique()
                        .HasDatabaseName("IX_LoyaltyAccounts_UserId");

                    b.ToTable("LoyaltyAccounts");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.LoyaltyTransaction", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<decimal>("Amount")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("NOW()");

                    b.Property<int>("LoyaltyAccountId")
                        .HasColumnType("integer");

                    b.Property<string>("Note")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<int?>("OrderId")
                        .HasColumnType("integer");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("character varying(30)");

                    b.HasKey("Id");

                    b.HasIndex("LoyaltyAccountId")
                        .HasDatabaseName("IX_LoyaltyTransactions_LoyaltyAccountId");

                    b.ToTable("LoyaltyTransactions");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.MenuCategory", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<int>("RestaurantId")
                        .HasColumnType("integer");

                    b.Property<int>("SortOrder")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("RestaurantId");

                    b.ToTable("MenuCategories");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Name = "Starters",
                            RestaurantId = 1,
                            SortOrder = 0
                        },
                        new
                        {
                            Id = 2,
                            Name = "Mains",
                            RestaurantId = 1,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 3,
                            Name = "Sides",
                            RestaurantId = 1,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 4,
                            Name = "Drinks",
                            RestaurantId = 1,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 5,
                            Name = "Starters",
                            RestaurantId = 2,
                            SortOrder = 0
                        },
                        new
                        {
                            Id = 6,
                            Name = "Mains",
                            RestaurantId = 2,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 7,
                            Name = "Sides",
                            RestaurantId = 2,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 8,
                            Name = "Drinks",
                            RestaurantId = 2,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 9,
                            Name = "Starters",
                            RestaurantId = 3,
                            SortOrder = 0
                        },
                        new
                        {
                            Id = 10,
                            Name = "Mains",
                            RestaurantId = 3,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 11,
                            Name = "Sides",
                            RestaurantId = 3,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 12,
                            Name = "Drinks",
                            RestaurantId = 3,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 13,
                            Name = "Starters",
                            RestaurantId = 4,
                            SortOrder = 0
                        },
                        new
                        {
                            Id = 14,
                            Name = "Mains",
                            RestaurantId = 4,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 15,
                            Name = "Sides",
                            RestaurantId = 4,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 16,
                            Name = "Drinks",
                            RestaurantId = 4,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 17,
                            Name = "Starters",
                            RestaurantId = 5,
                            SortOrder = 0
                        },
                        new
                        {
                            Id = 18,
                            Name = "Mains",
                            RestaurantId = 5,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 19,
                            Name = "Sides",
                            RestaurantId = 5,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 20,
                            Name = "Drinks",
                            RestaurantId = 5,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 21,
                            Name = "Starters",
                            RestaurantId = 6,
                            SortOrder = 0
                        },
                        new
                        {
                            Id = 22,
                            Name = "Mains",
                            RestaurantId = 6,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 23,
                            Name = "Sides",
                            RestaurantId = 6,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 24,
                            Name = "Drinks",
                            RestaurantId = 6,
                            SortOrder = 3
                        });
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.MenuItem", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Allergens")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<int>("CategoryId")
                        .HasColumnType("integer");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("NOW()");

                    b.Property<string>("Description")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("DietaryTags")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("ImageUrl")
                        .HasColumnType("text");

                    b.Property<bool>("IsAvailable")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("boolean");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<decimal>("Price")
                        .HasColumnType("decimal(10,2)");

                    b.Property<int>("RestaurantId")
                        .HasColumnType("integer");

                    b.Property<int?>("StockCount")
                        .HasColumnType("integer");

                    b.Property<bool>("TrackStock")
                        .HasColumnType("boolean");

                    b.HasKey("Id");

                    b.HasIndex("CategoryId");

                    b.HasIndex("RestaurantId");

                    b.ToTable("MenuItems");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Allergens = "[\"gluten\"]",
                            CategoryId = 1,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Crispy onion fritters",
                            DietaryTags = "[\"vegan\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Onion Bhaji",
                            Price = 4.50m,
                            RestaurantId = 1,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 2,
                            Allergens = "[\"gluten\"]",
                            CategoryId = 1,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Vegetable samosa (2 pieces)",
                            DietaryTags = "[\"vegan\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Samosa",
                            Price = 3.95m,
                            RestaurantId = 1,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 3,
                            Allergens = "[\"milk\"]",
                            CategoryId = 2,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Creamy tomato-based curry",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Chicken Tikka Masala",
                            Price = 11.95m,
                            RestaurantId = 1,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 4,
                            CategoryId = 2,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Slow-cooked lamb in rich sauce",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Lamb Rogan Josh",
                            Price = 13.50m,
                            RestaurantId = 1,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 5,
                            CategoryId = 2,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Chickpea curry",
                            DietaryTags = "[\"vegan\",\"gluten-free\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Chana Masala",
                            Price = 9.50m,
                            RestaurantId = 1,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 6,
                            CategoryId = 3,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            DietaryTags = "[\"vegan\",\"gluten-free\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Pilau Rice",
                            Price = 3.00m,
                            RestaurantId = 1,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 7,
                            Allergens = "[\"gluten\",\"milk\"]",
                            CategoryId = 3,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Garlic Naan",
                            Price = 2.50m,
                            RestaurantId = 1,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 8,
                            Allergens = "[\"milk\"]",
                            CategoryId = 4,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Mango Lassi",
                            Price = 3.50m,
                            RestaurantId = 1,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 9,
                            Allergens = "[\"gluten\"]",
                            CategoryId = 5,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Toasted bread, tomato, basil",
                            DietaryTags = "[\"vegan\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Bruschetta",
                            Price = 5.50m,
                            RestaurantId = 2,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 10,
                            Allergens = "[\"gluten\",\"milk\"]",
                            CategoryId = 5,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Fried risotto balls (3)",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Arancini",
                            Price = 6.50m,
                            RestaurantId = 2,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 11,
                            Allergens = "[\"gluten\",\"milk\"]",
                            CategoryId = 6,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Tomato, mozzarella, basil",
                            DietaryTags = "[\"vegetarian\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Margherita Pizza",
                            Price = 10.95m,
                            RestaurantId = 2,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 12,
                            Allergens = "[\"gluten\",\"milk\"]",
                            CategoryId = 6,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Spicy pepperoni, mozzarella",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Pepperoni Pizza",
                            Price = 12.95m,
                            RestaurantId = 2,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 13,
                            Allergens = "[\"gluten\",\"milk\",\"egg\"]",
                            CategoryId = 6,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Egg, pancetta, pecorino",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Spaghetti Carbonara",
                            Price = 12.50m,
                            RestaurantId = 2,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 14,
                            Allergens = "[\"gluten\",\"milk\"]",
                            CategoryId = 7,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Garlic Bread",
                            Price = 4.00m,
                            RestaurantId = 2,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 15,
                            Allergens = "[\"egg\",\"fish\",\"milk\"]",
                            CategoryId = 7,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Caesar Salad",
                            Price = 7.50m,
                            RestaurantId = 2,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 16,
                            CategoryId = 8,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            DietaryTags = "[\"vegan\",\"gluten-free\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "San Pellegrino",
                            Price = 2.95m,
                            RestaurantId = 2,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 17,
                            Allergens = "[\"soy\"]",
                            CategoryId = 9,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Steamed soy beans, sea salt",
                            DietaryTags = "[\"vegan\",\"gluten-free\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Edamame",
                            Price = 4.25m,
                            RestaurantId = 3,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 18,
                            Allergens = "[\"soy\"]",
                            CategoryId = 9,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Tofu, wakame, spring onion",
                            DietaryTags = "[\"vegan\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Miso Soup",
                            Price = 3.50m,
                            RestaurantId = 3,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 19,
                            Allergens = "[\"fish\"]",
                            CategoryId = 10,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Fresh sliced salmon",
                            DietaryTags = "[\"gluten-free\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Salmon Sashimi (8pc)",
                            Price = 14.50m,
                            RestaurantId = 3,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 20,
                            Allergens = "[\"fish\",\"soy\",\"sesame\"]",
                            CategoryId = 10,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Eel, avocado, cucumber",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Dragon Roll",
                            Price = 13.95m,
                            RestaurantId = 3,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 21,
                            Allergens = "[\"gluten\",\"egg\"]",
                            CategoryId = 10,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Crispy chicken, Japanese curry",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Chicken Katsu Curry",
                            Price = 11.95m,
                            RestaurantId = 3,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 22,
                            CategoryId = 11,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            DietaryTags = "[\"vegan\",\"gluten-free\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Steamed Rice",
                            Price = 2.50m,
                            RestaurantId = 3,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 23,
                            Allergens = "[\"gluten\",\"soy\"]",
                            CategoryId = 11,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Pork dumplings",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Gyoza (5pc)",
                            Price = 5.95m,
                            RestaurantId = 3,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 24,
                            CategoryId = 12,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            DietaryTags = "[\"vegan\",\"gluten-free\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Green Tea",
                            Price = 2.25m,
                            RestaurantId = 3,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 25,
                            Allergens = "[\"milk\"]",
                            CategoryId = 13,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Spicy chicken wings, blue cheese",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Buffalo Wings (6)",
                            Price = 6.95m,
                            RestaurantId = 4,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 26,
                            Allergens = "[\"gluten\",\"milk\"]",
                            CategoryId = 13,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            DietaryTags = "[\"vegetarian\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Mozzarella Sticks",
                            Price = 5.50m,
                            RestaurantId = 4,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 27,
                            Allergens = "[\"gluten\",\"milk\"]",
                            CategoryId = 14,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Beef, cheddar, lettuce, pickle",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Classic Cheeseburger",
                            Price = 9.95m,
                            RestaurantId = 4,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 28,
                            Allergens = "[\"gluten\",\"milk\"]",
                            CategoryId = 14,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Beef, bacon, BBQ, onion ring",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "BBQ Bacon Burger",
                            Price = 11.50m,
                            RestaurantId = 4,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 29,
                            Allergens = "[\"gluten\",\"soy\"]",
                            CategoryId = 14,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Plant patty, vegan cheese",
                            DietaryTags = "[\"vegan\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Beyond Vegan Burger",
                            Price = 10.95m,
                            RestaurantId = 4,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 30,
                            Allergens = "[\"milk\"]",
                            CategoryId = 15,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Cheese, bacon, jalapeños",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Loaded Fries",
                            Price = 5.50m,
                            RestaurantId = 4,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 31,
                            Allergens = "[\"gluten\"]",
                            CategoryId = 15,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            DietaryTags = "[\"vegan\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Onion Rings",
                            Price = 3.95m,
                            RestaurantId = 4,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 32,
                            Allergens = "[\"milk\"]",
                            CategoryId = 16,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Chocolate Milkshake",
                            Price = 4.50m,
                            RestaurantId = 4,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 33,
                            Allergens = "[\"gluten\",\"sesame\"]",
                            CategoryId = 17,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Chickpea hummus, warm pita",
                            DietaryTags = "[\"vegan\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Hummus & Pita",
                            Price = 4.95m,
                            RestaurantId = 5,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 34,
                            Allergens = "[\"gluten\"]",
                            CategoryId = 17,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Sourdough, avo, chilli flakes",
                            DietaryTags = "[\"vegan\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Avocado Toast",
                            Price = 6.50m,
                            RestaurantId = 5,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 35,
                            Allergens = "[\"sesame\"]",
                            CategoryId = 18,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Quinoa, kale, beetroot, tahini",
                            DietaryTags = "[\"vegan\",\"gluten-free\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Quinoa Buddha Bowl",
                            Price = 10.95m,
                            RestaurantId = 5,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 36,
                            Allergens = "[\"fish\",\"soy\"]",
                            CategoryId = 18,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Salmon, brown rice, edamame",
                            DietaryTags = "[\"gluten-free\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Grilled Salmon Bowl",
                            Price = 13.50m,
                            RestaurantId = 5,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 37,
                            Allergens = "[\"gluten\",\"sesame\"]",
                            CategoryId = 18,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Falafel, salad, tahini wrap",
                            DietaryTags = "[\"vegan\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Falafel Wrap",
                            Price = 8.95m,
                            RestaurantId = 5,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 38,
                            CategoryId = 19,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            DietaryTags = "[\"vegan\",\"gluten-free\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Sweet Potato Fries",
                            Price = 4.25m,
                            RestaurantId = 5,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 39,
                            CategoryId = 19,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            DietaryTags = "[\"vegan\",\"gluten-free\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Side Salad",
                            Price = 3.50m,
                            RestaurantId = 5,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 40,
                            CategoryId = 20,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Apple, ginger, lemon",
                            DietaryTags = "[\"vegan\",\"gluten-free\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Cold-Pressed Juice",
                            Price = 4.95m,
                            RestaurantId = 5,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 41,
                            Allergens = "[\"gluten\",\"soy\"]",
                            CategoryId = 21,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Vegetable spring rolls",
                            DietaryTags = "[\"vegan\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Spring Rolls (4)",
                            Price = 4.50m,
                            RestaurantId = 6,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 42,
                            Allergens = "[\"gluten\",\"soy\",\"sesame\"]",
                            CategoryId = 21,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Quarter duck, pancakes, hoisin",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Crispy Duck Pancakes",
                            Price = 12.95m,
                            RestaurantId = 6,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 43,
                            Allergens = "[\"gluten\",\"egg\"]",
                            CategoryId = 22,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Battered chicken, peppers",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Sweet & Sour Chicken",
                            Price = 9.95m,
                            RestaurantId = 6,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 44,
                            Allergens = "[\"peanut\",\"soy\",\"gluten\"]",
                            CategoryId = 22,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Beef, peanuts, Szechuan",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Kung Pao Beef",
                            Price = 11.50m,
                            RestaurantId = 6,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 45,
                            Allergens = "[\"soy\",\"gluten\"]",
                            CategoryId = 22,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Soft tofu, chilli bean sauce",
                            DietaryTags = "[\"vegetarian\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Mapo Tofu",
                            Price = 9.50m,
                            RestaurantId = 6,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 46,
                            Allergens = "[\"egg\",\"soy\"]",
                            CategoryId = 23,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Egg Fried Rice",
                            Price = 3.50m,
                            RestaurantId = 6,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 47,
                            Allergens = "[\"egg\",\"soy\",\"shellfish\"]",
                            CategoryId = 23,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            Description = "Curry vermicelli noodles",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Singapore Noodles",
                            Price = 8.95m,
                            RestaurantId = 6,
                            TrackStock = false
                        },
                        new
                        {
                            Id = 48,
                            CategoryId = 24,
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            DietaryTags = "[\"vegan\",\"gluten-free\"]",
                            IsAvailable = true,
                            IsDeleted = false,
                            Name = "Jasmine Tea",
                            Price = 2.00m,
                            RestaurantId = 6,
                            TrackStock = false
                        });
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.Order", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CancellableUntil")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("NOW()");

                    b.Property<DateTime?>("DeliveredAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("DeliveryAddressLine1")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("character varying(300)");

                    b.Property<string>("DeliveryCity")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<decimal>("DeliveryFee")
                        .HasColumnType("decimal(10,2)");

                    b.Property<string>("DeliveryPostcode")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<decimal>("DiscountAmount")
                        .HasColumnType("numeric");

                    b.Property<string>("DisputeNotes")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<string>("DisputeStatus")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("None");

                    b.Property<DateTime?>("EstimatedDeliveryTime")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("GiftCardCode")
                        .HasColumnType("text");

                    b.Property<decimal>("GiftCardDiscount")
                        .HasColumnType("numeric");

                    b.Property<string>("IdempotencyKey")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("OrderType")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<decimal>("PlusDiscount")
                        .HasColumnType("decimal(10,2)");

                    b.Property<string>("PromoCode")
                        .HasColumnType("text");

                    b.Property<string>("RejectionReason")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<int>("RestaurantId")
                        .HasColumnType("integer");

                    b.Property<uint>("RowVersion")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.Property<DateTime?>("ScheduledFor")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("SpecialInstructions")
                        .HasColumnType("text");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("Pending");

                    b.Property<string>("StripePaymentIntentId")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<decimal>("TotalAmount")
                        .HasColumnType("decimal(10,2)");

                    b.Property<int>("UserId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("IdempotencyKey", "UserId")
                        .IsUnique()
                        .HasDatabaseName("IX_Orders_IdempotencyKey_UserId");

                    b.HasIndex("RestaurantId", "Status")
                        .HasDatabaseName("IX_Orders_RestaurantId_Status");

                    b.HasIndex("UserId", "CreatedAt")
                        .IsDescending(false, true)
                        .HasDatabaseName("IX_Orders_UserId_CreatedAt_Desc");

                    b.ToTable("Orders");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.OrderItem", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int?>("MenuItemId")
                        .HasColumnType("integer");

                    b.Property<int>("OrderId")
                        .HasColumnType("integer");

                    b.Property<int>("Quantity")
                        .HasColumnType("integer");

                    b.Property<decimal>("UnitPrice")
                        .HasColumnType("decimal(10,2)");

                    b.HasKey("Id");

                    b.HasIndex("MenuItemId");

                    b.HasIndex("OrderId");

                    b.ToTable("OrderItems");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.PlatformSetting", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Key")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Value")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("PlatformSettings");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.ProcessedStripeEvent", b =>
                {
                    b.Property<string>("EventId")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<DateTime>("ProcessedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("NOW()");

                    b.HasKey("EventId");

                    b.ToTable("ProcessedStripeEvents");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.PromoCode", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("NOW()");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("DiscountType")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<decimal>("DiscountValue")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.Property<DateTime?>("ExpiresAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<int?>("MaxUses")
                        .HasColumnType("integer");

                    b.Property<decimal?>("MinOrderAmount")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.Property<int>("UsedCount")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("Code")
                        .IsUnique()
                        .HasDatabaseName("IX_PromoCodes_Code");

                    b.ToTable("PromoCodes");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.PromoCodeUsage", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<decimal>("DiscountApplied")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.Property<int>("OrderId")
                        .HasColumnType("integer");

                    b.Property<int>("PromoCodeId")
                        .HasColumnType("integer");

                    b.Property<DateTime>("UsedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("NOW()");

                    b.Property<int>("UserId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("OrderId");

                    b.HasIndex("PromoCodeId");

                    b.HasIndex("UserId", "PromoCodeId")
                        .HasDatabaseName("IX_PromoCodeUsages_User_Code");

                    b.ToTable("PromoCodeUsages");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.Restaurant", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("AngelcamCameraId")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("BasePostcode")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("NOW()");

                    b.Property<string>("CuisineType")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<double>("DeliveryRadiusMiles")
                        .HasColumnType("double precision");

                    b.Property<int>("EstimatedDeliveryMinutes")
                        .HasColumnType("integer");

                    b.Property<int>("HygieneRating")
                        .HasColumnType("integer");

                    b.Property<string>("ImageUrl")
                        .HasColumnType("text");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<string>("KitchenVideoUrl")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("Phone")
                        .HasColumnType("text");

                    b.Property<string>("StripeAccountId")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<bool>("SupportsCollection")
                        .HasColumnType("boolean");

                    b.HasKey("Id");

                    b.ToTable("Restaurants");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Address = "42 High Street, Westminster, London",
                            AngelcamCameraId = "133627",
                            BasePostcode = "SW1A 1AA",
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            CuisineType = "Other",
                            DeliveryRadiusMiles = 3.0,
                            EstimatedDeliveryMinutes = 30,
                            HygieneRating = 5,
                            IsActive = true,
                            Name = "Spice Garden",
                            SupportsCollection = false
                        },
                        new
                        {
                            Id = 2,
                            Address = "18 Soho Square, London",
                            BasePostcode = "W1D 3QL",
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            CuisineType = "Other",
                            DeliveryRadiusMiles = 2.5,
                            EstimatedDeliveryMinutes = 30,
                            HygieneRating = 5,
                            IsActive = true,
                            Name = "Bella Napoli",
                            SupportsCollection = false
                        },
                        new
                        {
                            Id = 3,
                            Address = "7 Shoreditch High Street, London",
                            BasePostcode = "E1 6JE",
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            CuisineType = "Other",
                            DeliveryRadiusMiles = 4.0,
                            EstimatedDeliveryMinutes = 30,
                            HygieneRating = 5,
                            IsActive = true,
                            Name = "Sakura Sushi",
                            SupportsCollection = false
                        },
                        new
                        {
                            Id = 4,
                            Address = "55 Camden High Street, London",
                            BasePostcode = "NW1 7JH",
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            CuisineType = "Other",
                            DeliveryRadiusMiles = 3.5,
                            EstimatedDeliveryMinutes = 30,
                            HygieneRating = 4,
                            IsActive = true,
                            Name = "The Burger Joint",
                            SupportsCollection = false
                        },
                        new
                        {
                            Id = 5,
                            Address = "12 Notting Hill Gate, London",
                            BasePostcode = "W11 3HR",
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            CuisineType = "Other",
                            DeliveryRadiusMiles = 3.0,
                            EstimatedDeliveryMinutes = 30,
                            HygieneRating = 5,
                            IsActive = true,
                            Name = "Green Bowl",
                            SupportsCollection = false
                        },
                        new
                        {
                            Id = 6,
                            Address = "88 Gerrard Street, Chinatown, London",
                            BasePostcode = "W1D 5PT",
                            CreatedAt = new DateTime(2026, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc),
                            CuisineType = "Other",
                            DeliveryRadiusMiles = 2.5,
                            EstimatedDeliveryMinutes = 30,
                            HygieneRating = 4,
                            IsActive = true,
                            Name = "Dragon Wok",
                            SupportsCollection = false
                        });
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.RestaurantHours", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<TimeSpan>("CloseTime")
                        .HasColumnType("interval");

                    b.Property<int>("DayOfWeek")
                        .HasColumnType("integer");

                    b.Property<bool>("IsClosed")
                        .HasColumnType("boolean");

                    b.Property<TimeSpan>("OpenTime")
                        .HasColumnType("interval");

                    b.Property<int>("RestaurantId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("RestaurantId");

                    b.ToTable("RestaurantHours");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 0,
                            IsClosed = true,
                            OpenTime = new TimeSpan(0, 11, 0, 0, 0),
                            RestaurantId = 1
                        },
                        new
                        {
                            Id = 2,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 1,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 11, 0, 0, 0),
                            RestaurantId = 1
                        },
                        new
                        {
                            Id = 3,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 2,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 11, 0, 0, 0),
                            RestaurantId = 1
                        },
                        new
                        {
                            Id = 4,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 3,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 11, 0, 0, 0),
                            RestaurantId = 1
                        },
                        new
                        {
                            Id = 5,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 4,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 11, 0, 0, 0),
                            RestaurantId = 1
                        },
                        new
                        {
                            Id = 6,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 5,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 11, 0, 0, 0),
                            RestaurantId = 1
                        },
                        new
                        {
                            Id = 7,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 6,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 11, 0, 0, 0),
                            RestaurantId = 1
                        },
                        new
                        {
                            Id = 8,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 0,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 2
                        },
                        new
                        {
                            Id = 9,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 1,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 2
                        },
                        new
                        {
                            Id = 10,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 2,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 2
                        },
                        new
                        {
                            Id = 11,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 3,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 2
                        },
                        new
                        {
                            Id = 12,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 4,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 2
                        },
                        new
                        {
                            Id = 13,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 5,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 2
                        },
                        new
                        {
                            Id = 14,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 6,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 2
                        },
                        new
                        {
                            Id = 15,
                            CloseTime = new TimeSpan(0, 22, 0, 0, 0),
                            DayOfWeek = 0,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 3
                        },
                        new
                        {
                            Id = 16,
                            CloseTime = new TimeSpan(0, 22, 0, 0, 0),
                            DayOfWeek = 1,
                            IsClosed = true,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 3
                        },
                        new
                        {
                            Id = 17,
                            CloseTime = new TimeSpan(0, 22, 0, 0, 0),
                            DayOfWeek = 2,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 3
                        },
                        new
                        {
                            Id = 18,
                            CloseTime = new TimeSpan(0, 22, 0, 0, 0),
                            DayOfWeek = 3,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 3
                        },
                        new
                        {
                            Id = 19,
                            CloseTime = new TimeSpan(0, 22, 0, 0, 0),
                            DayOfWeek = 4,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 3
                        },
                        new
                        {
                            Id = 20,
                            CloseTime = new TimeSpan(0, 22, 0, 0, 0),
                            DayOfWeek = 5,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 3
                        },
                        new
                        {
                            Id = 21,
                            CloseTime = new TimeSpan(0, 22, 0, 0, 0),
                            DayOfWeek = 6,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 3
                        },
                        new
                        {
                            Id = 22,
                            CloseTime = new TimeSpan(0, 23, 59, 0, 0),
                            DayOfWeek = 0,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 11, 0, 0, 0),
                            RestaurantId = 4
                        },
                        new
                        {
                            Id = 23,
                            CloseTime = new TimeSpan(0, 23, 59, 0, 0),
                            DayOfWeek = 1,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 11, 0, 0, 0),
                            RestaurantId = 4
                        },
                        new
                        {
                            Id = 24,
                            CloseTime = new TimeSpan(0, 23, 59, 0, 0),
                            DayOfWeek = 2,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 11, 0, 0, 0),
                            RestaurantId = 4
                        },
                        new
                        {
                            Id = 25,
                            CloseTime = new TimeSpan(0, 23, 59, 0, 0),
                            DayOfWeek = 3,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 11, 0, 0, 0),
                            RestaurantId = 4
                        },
                        new
                        {
                            Id = 26,
                            CloseTime = new TimeSpan(0, 23, 59, 0, 0),
                            DayOfWeek = 4,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 11, 0, 0, 0),
                            RestaurantId = 4
                        },
                        new
                        {
                            Id = 27,
                            CloseTime = new TimeSpan(0, 23, 59, 0, 0),
                            DayOfWeek = 5,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 11, 0, 0, 0),
                            RestaurantId = 4
                        },
                        new
                        {
                            Id = 28,
                            CloseTime = new TimeSpan(0, 23, 59, 0, 0),
                            DayOfWeek = 6,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 11, 0, 0, 0),
                            RestaurantId = 4
                        },
                        new
                        {
                            Id = 29,
                            CloseTime = new TimeSpan(0, 21, 0, 0, 0),
                            DayOfWeek = 0,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 9, 0, 0, 0),
                            RestaurantId = 5
                        },
                        new
                        {
                            Id = 30,
                            CloseTime = new TimeSpan(0, 21, 0, 0, 0),
                            DayOfWeek = 1,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 9, 0, 0, 0),
                            RestaurantId = 5
                        },
                        new
                        {
                            Id = 31,
                            CloseTime = new TimeSpan(0, 21, 0, 0, 0),
                            DayOfWeek = 2,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 9, 0, 0, 0),
                            RestaurantId = 5
                        },
                        new
                        {
                            Id = 32,
                            CloseTime = new TimeSpan(0, 21, 0, 0, 0),
                            DayOfWeek = 3,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 9, 0, 0, 0),
                            RestaurantId = 5
                        },
                        new
                        {
                            Id = 33,
                            CloseTime = new TimeSpan(0, 21, 0, 0, 0),
                            DayOfWeek = 4,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 9, 0, 0, 0),
                            RestaurantId = 5
                        },
                        new
                        {
                            Id = 34,
                            CloseTime = new TimeSpan(0, 21, 0, 0, 0),
                            DayOfWeek = 5,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 9, 0, 0, 0),
                            RestaurantId = 5
                        },
                        new
                        {
                            Id = 35,
                            CloseTime = new TimeSpan(0, 21, 0, 0, 0),
                            DayOfWeek = 6,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 9, 0, 0, 0),
                            RestaurantId = 5
                        },
                        new
                        {
                            Id = 36,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 0,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 6
                        },
                        new
                        {
                            Id = 37,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 1,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 6
                        },
                        new
                        {
                            Id = 38,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 2,
                            IsClosed = true,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 6
                        },
                        new
                        {
                            Id = 39,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 3,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 6
                        },
                        new
                        {
                            Id = 40,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 4,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 6
                        },
                        new
                        {
                            Id = 41,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 5,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 6
                        },
                        new
                        {
                            Id = 42,
                            CloseTime = new TimeSpan(0, 23, 0, 0, 0),
                            DayOfWeek = 6,
                            IsClosed = false,
                            OpenTime = new TimeSpan(0, 12, 0, 0, 0),
                            RestaurantId = 6
                        });
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.RestaurantPromotion", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int?>("AppliesToCategoryId")
                        .HasColumnType("integer");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("NOW()");

                    b.Property<string>("Description")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("DiscountType")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<decimal>("DiscountValue")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.Property<DateTime?>("EndsAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<int>("RestaurantId")
                        .HasColumnType("integer");

                    b.Property<DateTime?>("StartsAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.HasKey("Id");

                    b.HasIndex("AppliesToCategoryId");

                    b.HasIndex("RestaurantId")
                        .HasDatabaseName("IX_RestaurantPromotions_RestaurantId");

                    b.ToTable("RestaurantPromotions");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.Review", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Comment")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("NOW()");

                    b.Property<int>("CustomerId")
                        .HasColumnType("integer");

                    b.Property<int>("OrderId")
                        .HasColumnType("integer");

                    b.Property<int>("RestaurantId")
                        .HasColumnType("integer");

                    b.Property<int>("Stars")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("CustomerId");

                    b.HasIndex("OrderId")
                        .IsUnique()
                        .HasDatabaseName("IX_Reviews_OrderId_Unique");

                    b.HasIndex("RestaurantId", "CreatedAt")
                        .IsDescending(false, true)
                        .HasDatabaseName("IX_Reviews_RestaurantId_CreatedAt_Desc");

                    b.ToTable("Reviews");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.Subscription", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("NOW()");

                    b.Property<DateTime>("PeriodEnd")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<string>("StripeCustomerId")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<string>("StripeSubscriptionId")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<int>("UserId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("StripeSubscriptionId")
                        .IsUnique()
                        .HasDatabaseName("IX_Subscriptions_StripeId");

                    b.HasIndex("UserId")
                        .HasDatabaseName("IX_Subscriptions_UserId");

                    b.ToTable("Subscriptions");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.User", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("AuthProvider")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("NOW()");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<bool>("IsEmailVerified")
                        .HasColumnType("boolean");

                    b.Property<string>("OtpCode")
                        .HasColumnType("text");

                    b.Property<DateTime?>("OtpExpiresAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("OtpSentAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("PasswordHash")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("ProviderId")
                        .HasColumnType("text");

                    b.Property<int?>("RestaurantId")
                        .HasColumnType("integer");

                    b.Property<string>("Role")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<string>("Username")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("Id");

                    b.HasIndex("Email")
                        .IsUnique();

                    b.HasIndex("RestaurantId");

                    b.HasIndex("Username")
                        .IsUnique();

                    b.ToTable("Users");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.DataProtection.EntityFrameworkCore.DataProtectionKey", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("FriendlyName")
                        .HasColumnType("text");

                    b.Property<string>("Xml")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("DataProtectionKeys");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.Auction", b =>
                {
                    b.HasOne("FoodPlatform.Api.Data.Entities.MenuItem", "MenuItem")
                        .WithMany()
                        .HasForeignKey("MenuItemId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("FoodPlatform.Api.Data.Entities.Restaurant", "Restaurant")
                        .WithMany()
                        .HasForeignKey("RestaurantId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("FoodPlatform.Api.Data.Entities.User", "WinningUser")
                        .WithMany()
                        .HasForeignKey("WinningUserId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("MenuItem");

                    b.Navigation("Restaurant");

                    b.Navigation("WinningUser");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.Bid", b =>
                {
                    b.HasOne("FoodPlatform.Api.Data.Entities.Auction", "Auction")
                        .WithMany("Bids")
                        .HasForeignKey("AuctionId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("FoodPlatform.Api.Data.Entities.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Auction");

                    b.Navigation("User");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.FavouriteRestaurant", b =>
                {
                    b.HasOne("FoodPlatform.Api.Data.Entities.Restaurant", "Restaurant")
                        .WithMany()
                        .HasForeignKey("RestaurantId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("FoodPlatform.Api.Data.Entities.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Restaurant");

                    b.Navigation("User");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.GiftCard", b =>
                {
                    b.HasOne("FoodPlatform.Api.Data.Entities.User", "PurchasedByUser")
                        .WithMany()
                        .HasForeignKey("PurchasedByUserId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("PurchasedByUser");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.GiftCardUsage", b =>
                {
                    b.HasOne("FoodPlatform.Api.Data.Entities.GiftCard", "GiftCard")
                        .WithMany("Usages")
                        .HasForeignKey("GiftCardId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("FoodPlatform.Api.Data.Entities.Order", "Order")
                        .WithMany()
                        .HasForeignKey("OrderId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("GiftCard");

                    b.Navigation("Order");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.LoyaltyAccount", b =>
                {
                    b.HasOne("FoodPlatform.Api.Data.Entities.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.LoyaltyTransaction", b =>
                {
                    b.HasOne("FoodPlatform.Api.Data.Entities.LoyaltyAccount", "LoyaltyAccount")
                        .WithMany("Transactions")
                        .HasForeignKey("LoyaltyAccountId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("LoyaltyAccount");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.MenuCategory", b =>
                {
                    b.HasOne("FoodPlatform.Api.Data.Entities.Restaurant", "Restaurant")
                        .WithMany("MenuCategories")
                        .HasForeignKey("RestaurantId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Restaurant");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.MenuItem", b =>
                {
                    b.HasOne("FoodPlatform.Api.Data.Entities.MenuCategory", "Category")
                        .WithMany("Items")
                        .HasForeignKey("CategoryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("FoodPlatform.Api.Data.Entities.Restaurant", "Restaurant")
                        .WithMany("MenuItems")
                        .HasForeignKey("RestaurantId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Category");

                    b.Navigation("Restaurant");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.Order", b =>
                {
                    b.HasOne("FoodPlatform.Api.Data.Entities.Restaurant", "Restaurant")
                        .WithMany("Orders")
                        .HasForeignKey("RestaurantId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("FoodPlatform.Api.Data.Entities.User", "User")
                        .WithMany("Orders")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Restaurant");

                    b.Navigation("User");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.OrderItem", b =>
                {
                    b.HasOne("FoodPlatform.Api.Data.Entities.MenuItem", "MenuItem")
                        .WithMany()
                        .HasForeignKey("MenuItemId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("FoodPlatform.Api.Data.Entities.Order", "Order")
                        .WithMany("Items")
                        .HasForeignKey("OrderId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("MenuItem");

                    b.Navigation("Order");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.PromoCodeUsage", b =>
                {
                    b.HasOne("FoodPlatform.Api.Data.Entities.Order", "Order")
                        .WithMany()
                        .HasForeignKey("OrderId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("FoodPlatform.Api.Data.Entities.PromoCode", "PromoCode")
                        .WithMany("Usages")
                        .HasForeignKey("PromoCodeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("FoodPlatform.Api.Data.Entities.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Order");

                    b.Navigation("PromoCode");

                    b.Navigation("User");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.RestaurantHours", b =>
                {
                    b.HasOne("FoodPlatform.Api.Data.Entities.Restaurant", "Restaurant")
                        .WithMany("Hours")
                        .HasForeignKey("RestaurantId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Restaurant");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.RestaurantPromotion", b =>
                {
                    b.HasOne("FoodPlatform.Api.Data.Entities.MenuCategory", "AppliesToCategory")
                        .WithMany()
                        .HasForeignKey("AppliesToCategoryId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("FoodPlatform.Api.Data.Entities.Restaurant", "Restaurant")
                        .WithMany()
                        .HasForeignKey("RestaurantId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("AppliesToCategory");

                    b.Navigation("Restaurant");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.Review", b =>
                {
                    b.HasOne("FoodPlatform.Api.Data.Entities.User", "Customer")
                        .WithMany()
                        .HasForeignKey("CustomerId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("FoodPlatform.Api.Data.Entities.Order", "Order")
                        .WithMany()
                        .HasForeignKey("OrderId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("FoodPlatform.Api.Data.Entities.Restaurant", "Restaurant")
                        .WithMany()
                        .HasForeignKey("RestaurantId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Customer");

                    b.Navigation("Order");

                    b.Navigation("Restaurant");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.Subscription", b =>
                {
                    b.HasOne("FoodPlatform.Api.Data.Entities.User", "User")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.User", b =>
                {
                    b.HasOne("FoodPlatform.Api.Data.Entities.Restaurant", "Restaurant")
                        .WithMany("Staff")
                        .HasForeignKey("RestaurantId");

                    b.Navigation("Restaurant");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.Auction", b =>
                {
                    b.Navigation("Bids");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.GiftCard", b =>
                {
                    b.Navigation("Usages");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.LoyaltyAccount", b =>
                {
                    b.Navigation("Transactions");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.MenuCategory", b =>
                {
                    b.Navigation("Items");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.Order", b =>
                {
                    b.Navigation("Items");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.PromoCode", b =>
                {
                    b.Navigation("Usages");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.Restaurant", b =>
                {
                    b.Navigation("Hours");

                    b.Navigation("MenuCategories");

                    b.Navigation("MenuItems");

                    b.Navigation("Orders");

                    b.Navigation("Staff");
                });

            modelBuilder.Entity("FoodPlatform.Api.Data.Entities.User", b =>
                {
                    b.Navigation("Orders");
                });
#pragma warning restore 612, 618
        }
    }
}
